...
一个 `nslab.yaml` 同时演示两种常见用法，并把它们接入同一个 VLAN 拓扑：`h1`、`h2`
使用 VLAN 10 子接口接入 tagged trunk，`h10` 使用 VLAN 10 access 端口，`r1` 通过一条
trunk 提供 VLAN 10/20 子接口，`h20` 位于 VLAN 20。这样可以先验证同 VLAN 二层互通，
再验证经过 `r1` 的跨 VLAN 三层转发。
...
`h1` 和 `h2` 的 `eth0` 连接到 `sw1` 的 `trunk1`、`trunk2`，只承载带 tag 的帧；IPv4
地址配置在各自的 `vlan10` 上：
...
`sw1` 的 `access10`、`access20` 端口向主机发送 untagged 帧，`trunk1`、`trunk2` 和
`router` 端口保留 VLAN 10 的 tag；`router` 还承载 VLAN 20，`r1` 用两个 VLAN 子接口
完成三层转发：
