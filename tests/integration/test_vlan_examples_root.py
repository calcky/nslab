...
@pytest.mark.parametrize(
    ("example", "source", "destination", "devices"),
    [
        (
            "vlan-subinterface",
            "h1",
            "192.0.2.2",
            {"h1": {"vlan10": ("eth0", 10)}, "h2": {"vlan10": ("eth0", 10)}},
        ),
        (
            "router-on-a-stick",
            "h10",
            "192.168.20.2",
            {"r1": {"vlan10": ("eth0", 10), "vlan20": ("eth0", 20)}},
        ),
    ],
)
def test_vlan_example_lifecycle_and_connectivity(
    tmp_path: Path,
    example: str,
    source: str,
    destination: str,
    devices: Mapping[str, Mapping[str, tuple[str, int]]],
) -> None:
    topology = _EXAMPLES / example / "nslab.yaml"
    ...
    try:
        ...
        for node_name, expected_devices in devices.items():
            namespace = inventory.namespaces[plan.nodes[node_name].namespace]
            for device_name, (parent, vlan_id) in expected_devices.items():
