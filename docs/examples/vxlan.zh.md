...
这个页面使用一个 `nslab.yaml` 同时演示两种 VXLAN 数据平面。一个共享的 underlay
bridge 连接四个 VTEP：`vtep1` 和 `vtep2` 使用 VNI 100 提供二层 bridge overlay，`r1`
和 `r2` 使用 VNI 200 提供独立三层 routed overlay。两种 overlay 一起部署和销毁。
...
VNI 100 在两台 bridge VTEP 上都加入 `br0`。未知帧通过静态 remote VTEP 的 FDB 条目发送
到另一侧主机：
...
VNI 200 是 `r1` 和 `r2` 上的独立接口。它承载 tunnel 地址，并在
`10.80.1.0/24` 与 `10.80.2.0/24` 两个 LAN 之间路由：
...
`underlay` 只承载外层 UDP 流量。`vxlan100` 有 bridge master，`vxlan200` 没有 bridge
master，可以直接承载 IP 地址。underlay MTU 为 1500 时，nslab 会为两个 VXLAN 接口自动
设置 1450 的 MTU。
...
[查看示例 README](https://github.com/calcky/nslab/blob/main/examples/vxlan/README.md)
