...
通过 `xdp1` 连接两个 IPv4 子网，在 `xdp1:eth0` 上依次加载四个 eBPF section：

- `xdp/pass`：统计收到的帧并执行 `XDP_PASS`。
- `xdp/drop`：丢弃 ICMP Echo Request，其他帧继续进入协议栈。
- `xdp/tx`：把 Echo Request 改成 Reply，并执行 `XDP_TX`。
...
Ubuntu 上安装编译器、libbpf 头文件和观察工具，然后编译对象：
...
加载 tx section，并记录 `h2` 内核 ICMP 计数器：
...
ping 成功，但 `xdp1` 的两个内核 ICMP 计数器不增加。Echo Request 没有进入 `xdp1` 的
IPv4/ICMP 协议栈，Reply 由 XDP 程序直接构造并发回。
...
[查看 nslab.yaml](https://github.com/calcky/nslab/blob/main/examples/xdp/nslab.yaml) ·
[查看 eBPF 源码](https://github.com/calcky/nslab/blob/main/examples/xdp/xdp_lab.c) ·
[查看示例 README](https://github.com/calcky/nslab/blob/main/examples/xdp/README.md)
